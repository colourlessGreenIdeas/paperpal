    ...
    def simplify_chunk(self, chunk: str, context: str, grade_level: str) -> str:
        ...
        try:
            ...
            if grade_level == "original":
                ...
                prompt = f"""Using the provided context and specific chunk, create a simplified version of the text with the same length of text as original text that would be suitable for {grade_desc}. The simplified version should:
                ...
                3. Preserve important information while adjusting the complexity level and preserving content length
                ...
                6. if code is present, simplify it to a level appropriate for {grade_desc}
                7. For tables, simplify them to a level appropriate for {grade_desc}
                8. Include examples or analogies suitable for {grade_desc} when helpful
                9. Do not repeat explanations that were already covered in previous sections
                10. If a concept was already explained in a previous section, you can refer to it briefly
                11. Focus on new information and concepts not covered in previous sections
                12. Do not add any additional text or your own thoughts
    ...
    def process_all_grade_levels(self, chunks: List[str], grade_levels: List[str]) -> Dict[str, List[str]]:
        ...
        with ThreadPoolExecutor(max_workers=min(5, len(grade_levels))) as executor:
            ...
            for i, chunk in enumerate(chunks):
                # Create context with a fixed sliding window of previous chunks
                context_parts = []
                
                # Always include the first 1000 chars for high-level context
                context_parts.append(chunks[0][:1000])
                
                # Add up to 2 most recent previous chunks (not including current chunk)
                if i > 0:
                    prev_chunks = chunks[max(0, i-2):i]  # This will get either 1 or 2 previous chunks
                    if prev_chunks:
                        context_parts.append("Recently processed content:")
